...
3. This **displays** a **list** of **all providers** within the Microsoft.Compute namespace, such as (but definitely not limited to):
   ...
   4. Disks
   5. Cloud Services
4. If we wanted to drill into the Virtual Machines **providers** a bit more we can **filter** it like:

       Get-AzProviderOperation -Name Microsoft.Compute/virtualMachines/*
5. Here we can finally see the available actions, for example, the following Action will allow you to Read the VM sizes available to a Virtual Machine:
