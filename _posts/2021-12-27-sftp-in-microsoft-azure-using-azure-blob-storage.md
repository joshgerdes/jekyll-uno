...
SFTP using an Azure Storage account does not support shared access signature (SAS) or Azure Active Directory (Azure AD) authentication for connecting SFTP clients. Instead, SFTP clients must use a password or a Secure Shell _(SSH)_ private essential credential.
